...
<?php		
		...
		mysql_select_db("rto");
		$rec=mysql_query("select * from permit where date='".$_REQUEST['date']."'");
		echo "<table border='1'>";
		echo "<tr><th>Category</th><th>Reg No.</th><th>Name</th><th>Date of Journey</th><th>Date of Return </th><th>Members </th><th>Source </th> <th>Destination </th><th>Type </th><th>Date </th></tr> ";
		while($row=mysql_fetch_array($rec))
		{
			echo "<tr><td>".$row['category']."</td><td>".$row['permit_no']."</td><td>".$row['cand_name']."</td><td>".$row['doj']."</td><td>".$row['dor']."</td> <td>".$row['members']."</td> <td>".$row['source']."</td> <td>".$row['destination']."</td><td>".$row['type']."</td> <td>".$row['date']."</td></tr>";
		}
