...
<?php		
		...
		mysql_select_db("rto");
		$rec=mysql_query("select * from driving_license where date='".$_REQUEST['date']."'");
		echo "<table border='1'>";
		echo "<tr><th>Category</th><th>Reg No.</th><Name</th><th>Sex</th><th>D.O.B </th><th>Father Name </th><th>Mother Name </th> <th>Qualification </th><th>Address </th><th>District </th><th>State </th><th>Pincode </th><th>Mobile </th><th>Email Id </th></tr> ";
		while($row=mysql_fetch_array($rec))
		{
			echo "<tr><td>".$row['category']."</td><td>".$row['dr_no']."</td><td>".$row['sex']."</td><td>".$row['dob']."</td><td>".$row['fname']."</td> <td>".$row['mname']."</td> <td>".$row['qualcode']."</td> <td>".$row['add1']."".$row['add2']."</td> <td>".$row['district']."</td> <td>".$row['state']."</td> <td>".$row['pincode']."</td> <td>".$row['mobile']."</td> <td>".$row['email']."</td></tr>";
		}
