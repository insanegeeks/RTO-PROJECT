...
<div id="middle">
	<div id="middle_left">
		...
		<div class="boxedD">
			<h2>Quick Links </h2>
			<ul>
				<li><a href="#">Download LLR Question Bank</a></li>
	...
	<div id="content_left">
		<div class="col_w460">
    	<h3>You have successfully applied for Learning Licence.</h3>
    	<h3>Please keep the hard copy of form and Note down registration number : <?php echo $_REQUEST['veh_no']; ?> </h3>
		<div class="Hline">&nbsp;</div>
		<div class="cleaner_h10"></div>
		<?php
		//for Bill upload
		if((($_FILES["bill"]["type"] == "image/gif") ||($_FILES["bill"]["type"] == "image/jpeg") || ($_FILES["bill"]["type"] == "image/pjpeg") || ($_FILES["bill"]["type"] == "image/png")) && ($_FILES["bill"]["size"]>0))
		{
			if($_FILES["bill"]["error"] > 0)
			{
				echo "Return Code : " . $_FILES["bill"]["error"] . "<br/>";
			}
			else
			{
				if(file_exists("bill/" . $_FILES["bill"]["name"]))
				{
					echo $_FILES["bill"]["name"] . "Already Exists...!!";
				}
				else
				{
					move_uploaded_file($_FILES["bill"]["tmp_name"],"bill/" . $_REQUEST['veh_no'].".jpg");
				}
			}
		}
		else
		...
		mysql_select_db("rto");
		$rec=mysql_query("select * from vehicle where veh_no=".$_REQUEST['veh_no']);

		echo "<center>";
		echo "<table border='1' width='85%'>";
		$row=mysql_fetch_array($rec);

		echo  "	<tr><td>Registration No.</td><td>".$row['veh_no']."</td></tr>
				<tr><td>Category</td><td>".$row['category']."</td></tr>
				<tr><td>Name</td><td>".$row['cand_name']."</td></tr>
				<tr><td>Nationality</td><td>".$row['nationality']."</td></tr>
				<tr><td>Address</td><td>".$row['add1']."</td></tr>
				<tr><td></td><td>".$row['add2']."</td></tr>
				<tr><td></td><td>".$row['po']."</td></tr>
				<tr><td>District</td><td>".$row['district']."</td></tr>
				<tr><td>Pincode</td><td>".$row['pincode']."</td></tr>
				<tr><td>Mobile</td><td>".$row['mobile']."</td></tr>
				<tr><td>Email</td><td>".$row['email']."</td></tr>
				<tr><td>Vehicle Bill</td><td><img src='photo/".$row['veh_no'].".jpg' height=200 width=200></td></tr>
