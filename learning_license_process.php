...
<div id="middle">
	<div id="middle_left">
		<div class="boxedB">
			...
			<ul  >
				...
				<li><a target="_blank" href="process_permit.html">National Permit</a></li>			</ul>
		</div>
		<div class="boxedD">
			<h2>Quick Links </h2>
			<ul>
	...
	<div id="content_left">
		<div class="col_w460">
    	<h3>Upload Photo, Signature, Identity Proof to Continue...!!</h3>
		<div class="Hline">&nbsp;</div>
		...
		if(!$con)
		die("Not Connected Please Try Later...!!".mysql_error());
		mysql_select_db("rto");

		$rec=mysql_query("select * from learning");
		if(mysql_num_rows($rec)==0)
		{
			$rec=mysql_query("select * from reg_no");
			$row=mysql_fetch_array($rec);
			$r=$row['reg_no'];
		}
		else
		{
			$rec=mysql_query("select * from learning order by reg_no desc");
			$row=mysql_fetch_array($rec);
			$r=$row['reg_no']+1;
		}
		
		mysql_query("insert into learning (category, cand_name, sex, dob, fname, mname, nationality, marital_code, religion, community, minority_status, qualcode, add1, add2, po, district, state, pincode, area_code, phone, mobile, email, confirmemail, date, reg_no) values ('".$_REQUEST['category']."', '".$_REQUEST['cand_name']."', '".$_REQUEST['sex']."', '".$_REQUEST['dob']."', '".$_REQUEST['fname']."', '".$_REQUEST['mname']."', '".$_REQUEST['nationality']."', '".$_REQUEST['marital_code']."', '".$_REQUEST['religion']."', '".$_REQUEST['community']."', '".$_REQUEST['minority_status']."', '".$_REQUEST['qualcode']."', '".$_REQUEST['add1']."', '".$_REQUEST['add2']."', '".$_REQUEST['po']."', '".$_REQUEST['district']."', '".$_REQUEST['state']."', '".$_REQUEST['pincode']."', '".$_REQUEST['area_code']."', '".$_REQUEST['phone']."',  '".$_REQUEST['mobile']."', '".$_REQUEST['email']."', '".$_REQUEST['confirmemail']."', '$date' , '$r')");
		
		mysql_query("insert into ll_status (category, cand_name, dob, email, date, reg_no) values ('".$_REQUEST['category']."', '".$_REQUEST['cand_name']."', '".$_REQUEST['dob']."', '".$_REQUEST['email']."', '$date' , '$r')");
		echo $r.'<input type="hidden" name="reg_no" value="'.$r.'"></td></tr>';
	    ?>

		<form action="learning_license_process1.php" method="post" enctype="multipart/form-data" name="upload_conent_form">
		<table align="center" cellspacing="6" width="75%" border="0">
		  <tbody>
		  <tr><td>
		  <?php
		  echo $r.'<input type="hidden" name="reg_no" value="'.$r.'"></td></tr>';
		  ...
		  <tr>
			<td colspan="3"><strong>Upload Photo, Signature and Identity Proof</strong> </td>
		  </tr>
		  <tr>
			<td width="25%" valign="top"><img src="images/formbullet.gif">&nbsp;&nbsp;<label for="photo">Photo</label><font color="red">*</font> : </td>
			<td colspan="2"><input type="file" name="photo" id="photo" /></td>
		  </tr>
		  <tr>
			<td width="25%" valign="top"><img src="images/formbullet.gif">&nbsp;&nbsp;<label for="sign">Signature</label><font color="red">*</font> : </td>
			<td colspan="2"><input type="file" name="sign" id="sign" /></td>
		  </tr>
		  <tr>
			<td width="25%" valign="top"><img src="images/formbullet.gif">&nbsp;&nbsp;<label for="proof1">Identity Proof</label><font color="red">*</font> : </td>
			<td colspan="2"><input type="file" name="proof1" id="proof1" /></td>
		  </tr>
		   <tr>
			<td width="25%" valign="top"><img src="images/formbullet.gif">&nbsp;&nbsp;<label for="proof2">Address Proof</label><font color="red">*</font> : </td>
			<td colspan="2"><input type="file" name="proof2" id="proof2" /></td>
		  </tr>
		  <tr>
		  <td></td>
		  <td><input type="reset" name="Reset" value="Reset"> <input name="Submit" type="Submit" value="Continue"></td>
		  </tr>
		  <tr><td colspan="2"><font color="red">(*) - Star marked fields are essentially to be filled by the candidate.</font>
		  </td></tr>
		</tbody></table>
		</form>	
		</div>	
...
<div id="footer_wrapper">
    <div id="footer">  
				...
				<h6>Copyright 2022 &#169; Online RTO Management</h6>
				</div>
				<div class="footer_right">  
				<h6>Design and Developed by InsaneGeeks</h6>
