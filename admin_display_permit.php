...
<div id="middle">	
	<div id="content_left">
		...
		<h3>Display All Requests</h3>
<?php		
		$con=mysql_connect("localhost","root","");
		if(!$con)
		die("Not Connected Please Try Again..!".mysql_error());
		mysql_select_db("rto");
		$rec=mysql_query("select * from permit");
		echo "<table border='1'>";
		echo "<tr><th>Category</th><th>Reg No.</th><Name</th><th>Date of Journey</th><th>Date of Return </th><th>Members </th><th>Source </th> <th>Destination </th><th>Type </th><th>Date </th></tr> ";
		while($row=mysql_fetch_array($rec))
		{
			echo "<tr><td>".$row['category']."</td><td>".$row['permit_no']."</td><td>".$row['cand_name']."</td><td>".$row['doj']."</td><td>".$row['dor']."</td> <td>".$row['members']."</td> <td>".$row['source']."</td> <td>".$row['destination']."".$row['type']."</td> <td>".$row['date']."</td></tr>";
		}
