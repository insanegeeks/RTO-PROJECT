...
<div id="middle">
	...
	<div id="content_left">
		...
		<?php
		echo "<h3>Today Application Received : ".$_REQUEST['date']."</h3>";
		?>
...
<?php		
		...
		mysql_select_db("rto");
		$rec=mysql_query("select * from vehicle where date='".$_REQUEST['date']."'");
		echo "<table border='1'>";
		echo "<tr><th>Category</th><th>Reg No.</th>Name</th> <th>Address </th><th>District </th><th>State </th><th>Pincode </th><th>Mobile </th><th>Dealer Name</th><th>Vehicle </th><th>Engine No. </th><th>Chassis </th> <th>Maker Name </th></tr> ";
		while($row=mysql_fetch_array($rec))
		{
			echo "<tr><td>".$row['category']."</td><td>".$row['veh_no']."</td> <td>".$row['cand_name']."</td> <td>".$row['oadd1']."".$row['oadd2']."</td> <td>".$row['district']."</td> <td>".$row['state']."</td> <td>".$row['pincode']."</td><td>".$row['mobile']."</td> <td>".$row['dealr_name']."</td> <td>".$row['veh_class']."</td> <td>".$row['eng_no']."</td> <td>".$row['chasis_no']."</td> <td>".$row['maker_name']."</td></tr>";
		}
